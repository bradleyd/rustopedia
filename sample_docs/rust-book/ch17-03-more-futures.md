...
Box
—and
we’re only doing that to get the types to line up. We don’t actually
need
the
...
The
a
future still runs for a bit before handing off control to
b
, because
it calls
slow
...
stream
part of the crate; it works just fine
