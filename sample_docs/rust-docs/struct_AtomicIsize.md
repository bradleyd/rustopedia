AtomicIsize in std::sync::atomic - Rust
std
::
sync
::
...
let
atomic_forty_two = AtomicIsize::new(
42
);
1.75.0 (const: 1.84.0)
·
...
ptr:
*mut
isize = Box::into_raw(Box::new(
0
));
assert!
(ptr.cast::<AtomicIsize>().is_aligned());
...
view:
&mut
[isize] = AtomicIsize::get_mut_slice(
&mut
some_ints);
assert_eq!
(view, [
...
AtomicIsize::compare_exchange_weak
,
